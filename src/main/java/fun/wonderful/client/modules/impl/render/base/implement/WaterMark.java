...
public class WaterMark extends InterfaceProcessing {
    ...
    public void DefaultStyle(EventRender.Default eventRender) {
        ...
        if (!Wonderful.INSTANCE.themeStorage.getThemes().getTheme().getName().equals("Rainbow")) {
            iconTop = Wonderful.INSTANCE.themeStorage.getThemes().getTheme().color[0];
        } else {
        ...
        float wonderfulRectX = x;
        float wonderfulRectY = y;
        float wonderfulRectW = brandTextX + brandTextW + 1.5f - x;

        RenderUtils.drawDefaultHudThemedPanel(matrices, wonderfulRectX, wonderfulRectY, wonderfulRectW, wonderfulRectH, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
        }
        ...
        RenderUtils.drawDefaultHudThemedPanel(matrices, rect2X, wonderfulRectY, rect2W, rect2H, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
        ...
        float rectBtmY = wonderfulRectY + wonderfulRectH + 2f;
        ...
        float totalW = Math.max(wonderfulRectW + 2f + rect2W, rectBtmW);
        draggable.setWidth(totalW);
        draggable.setHeight(showBottom ? (wonderfulRectH + 1f + rectBtmH) : wonderfulRectH);
    }
    ...
    public void WaveStyle(EventRender.Default eventRender) {
        ...
        String watermarkText = "Wonderful";
